...
//    }
//
//    //the original calculateEverything method
//    @Deprecated
...
//        launchPointToGoalCenterX_Distance_Inches = launchPointToGoalCenterX_Distance_Meters * 39.3700787;
//        turretAngle = -1*(Math.atan(posY/posX)-heading); //Switch to atan 2 to avoid asymptote issue
//    }
//
...
//        double newX=Math.sqrt(Math.pow(robotBaseX, 2)+Math.pow(tagToGoalCenter_Distance, 2)-2*robotBaseXtagToGoalCenter_Distance*Math.cos(Math.PI-actualTagYaw[0]));   //law of cosines. New X is equal to the distance from the robotBase to the goalCenter
